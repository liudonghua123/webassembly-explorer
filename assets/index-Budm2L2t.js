...
This is line 2
And line 3`),[c,l]=R(null),[d,o]=R(!1),[f,u]=R(null),[x,k]=R(null),[U,w]=R(!1),[b,O]=R("");ze(()=>{N(),e("wasmer")},[]),ze(()=>{const g=kt(t,n,i);O(g)},[t,n,i,a]);async function N(){o(!0),k(null);try{const g=await fetch("linecount.wasm");if(!g.ok)throw new Error("Failed to load WASM file. Please compile linecount.c first.");const m=await g.arrayBuffer();l(new Uint8Array(m)),w(!0)}catch(g){k(g.message),w(!1)}o(!1)}async function q(){o(!0),k(null),u(null);try{let g="",m=null,M=null,D=0,W=null,B=0,y=-1;const E=n==="stdin"?[]:["-i",i];if(n==="stdin")M=new TextEncoder().encode(a);else{const p=await fetch(`/${i}`);if(p.ok){const h=await p.arrayBuffer();W=new Uint8Array(h)}}const H=new TextEncoder,J=E.map(p=>H.encode(p+"\0")),X=J.reduce((p,h)=>p+h.length,0),ne={wasi_snapshot_preview1:{args_sizes_get:(p,h)=>(new Uint32Array(m.buffer,p,1)[0]=E.length,new Uint32Array(m.buffer,h,1)[0]=X,0),args_get:(p,h)=>{const $=new Uint32Array(m.buffer,p,E.length);let C=0;for(let I=0;I<E.length;I++){$[I]=h+C;const L=J[I];new Uint8Array(m.buffer,h+C,L.length).set(L),C+=L.length}return 0},environ_sizes_get:(p,h)=>(new Uint32Array(m.buffer,p,1)[0]=0,new Uint32Array(m.buffer,h,1)[0]=0,0),environ_get:()=>0,proc_exit:p=>{throw new Error(`Process exited: ${p}`)},fd_write:(p,h,$,C)=>{if(p===1){let I=0;for(let L=0;L<$;L++){const K=new Uint32Array(m.buffer,h+L*8,1)[0],z=new Uint32Array(m.buffer,h+L*8+4,1)[0];g+=new TextDecoder().decode(new Uint8Array(m.buffer,K,z)),I+=z}new Uint32Array(m.buffer,C,1)[0]=I,u({stdout:g.trim(),stderr:""})}return 0},fd_read:(p,h,$,C)=>{const I=n==="stdin"?M:p===y?W:null,L=n==="stdin"?D:p===y?B:0;if(I){let K=0;for(let z=0;z<$;z++){const we=new Uint32Array(m.buffer,h+z*8,1)[0],be=new Uint32Array(m.buffer,h+z*8+4,1)[0],ve=new Uint8Array(m.buffer,we,be),se=Math.min(be,I.length-L);for(let oe=0;oe<se;oe++)ve[oe]=I[L+oe];n==="stdin"?D+=se:B+=se,K+=se}return new Uint32Array(m.buffer,C,1)[0]=K,0}return 0},fd_seek:(p,h,$,C)=>{if(p===y&&W){let I;if($===0)I=h;else if($===1)I=B+h;else if($===2)I=W.length+h;else return-1;return B=I,C&&(new Uint32Array(m.buffer,C,1)[0]=B),0}return 0},fd_close:p=>(p===y&&(y=-1),0),fd_fdstat_get:(p,h)=>{const $=new Uint8Array(m.buffer,h,24);return p===0||p===1||p===2?$[0]=2:(p===3||p===y)&&($[0]=1),0},fd_filestat_get:(p,h)=>(p===y&&W&&(new BigUint64Array(m.buffer,h,6)[5]=BigInt(W.length)),0),fd_prestat_get:(p,h)=>p===3?(new Uint8Array(m.buffer,h,1)[0]=1,new Uint32Array(m.buffer,h+4,1)[0]=1,0):8,fd_prestat_dir_name:(p,h,$)=>p===3&&$>0?(new Uint8Array(m.buffer,h,1)[0]=46,0):8,path_open:(p,h,$,C,I,L,K,z,we)=>{const ve=new TextDecoder().decode(new Uint8Array(m.buffer,$,C)).split("/").pop();return n==="argument"&&W&&ve===i.split("/").pop()?(y=4,new Uint32Array(m.buffer,we,1)[0]=y,0):-1},path_filestat_get:(p,h,$,C,I)=>p===y&&W?(new BigUint64Array(m.buffer,I,6)[5]=BigInt(W.length),0):-1,clock_time_get:(p,h,$)=>(new BigUint64Array(m.buffer,$,1)[0]=BigInt(Date.now())*BigInt(1e6),0),clock_res_get:()=>0,random_get:(p,h)=>(crypto.getRandomValues(new Uint8Array(m.buffer,p,h)),0),fd_advise:()=>0,fd_allocate:()=>0,fd_datasync:()=>0,fd_fdstat_set_flags:()=>0,fd_fdstat_set_rights:()=>0,fd_filestat_set_size:()=>0,fd_filestat_set_times:()=>0,fd_pread:()=>0,fd_pwrite:()=>0,fd_readdir:()=>0,fd_renumber:()=>0,fd_sync:()=>0,fd_tell:()=>0,path_create_directory:()=>0,path_filestat_set_times:()=>0,path_link:()=>0,path_readlink:()=>0,path_remove_directory:()=>0,path_rename:()=>0,path_symlink:()=>0,path_unlink_file:()=>0,poll_oneoff:()=>0,sched_yield:()=>0,sock_accept:()=>0,sock_recv:()=>0,sock_send:()=>0,sock_shutdown:()=>0}},{instance:re}=await WebAssembly.instantiate(c.buffer,ne);m=re.exports.memory,re.exports._start()}catch(g){k(`WASM Runner Error: ${g.message}`)}o(!1)}async function te(){o(!0),k(null),u(null);try{const{WASI:g,File:m,OpenFile:M,ConsoleStdout:D,PreopenDirectory:W}=await Y(async()=>{const{WASI:y,File:E,OpenFile:H,ConsoleStdout:J,PreopenDirectory:X}=await import("./index-DlF0zFmt.js");return{WASI:y,File:E,OpenFile:H,ConsoleStdout:J,PreopenDirectory:X}},[]);let B="";if(n==="stdin"){const y=new g(["./linecount"],[],[new M(new m(new TextEncoder().encode(a))),D.lineBuffered(H=>{B+=H,u({stdout:B.trim(),stderr:""})}),D.lineBuffered(()=>{})]),{instance:E}=await WebAssembly.instantiate(c.buffer,{wasi_snapshot_preview1:y.wasiImport});y.start(E)}else{const y=await fetch(`/${i}`);if(!y.ok)throw new Error(`Failed to fetch file: ${i}`);const E=new Uint8Array(await y.arrayBuffer()),H=i.split("/"),J=H.pop(),X=H.join("/")||".",ne=new g(["./linecount","-i",i],[],[new M(new m([])),D.lineBuffered(p=>{B+=p,u({stdout:B.trim(),stderr:""})}),D.lineBuffered(()=>{}),new W(X,[[J,new m(E)]])]),{instance:re}=await WebAssembly.instantiate(c.buffer,{wasi_snapshot_preview1:ne.wasiImport});ne.start(re)}}catch(g){k(`Browser WASI Shim Error: ${g.message}`)}o(!1)}async function G(){if(window.__WASMER_INITIALIZED__)return;const{init:g,initializeLogger:m}=await Y(async()=>{const{init:W,initializeLogger:B}=await import("./index-CavDQnhK.js");return{init:W,initializeLogger:B}},[]),{default:M}=await Y(async()=>{const{default:W}=await import("./wasmer_js_bg-BXjoki3b.js");return{default:W}},[]),{default:D}=await Y(async()=>{const{default:W}=await import("./index-CyyMDdgM.js");return{default:W}},[]);await g({module:M,sdkUrl:new URL(D,window.location.origin).href}),m("debug"),window.__WASMER_INITIALIZED__=!0}async function Q(){o(!0),k(null),u(null);try{await G();const{runWasix:g}=await Y(async()=>{const{runWasix:E}=await import("./index-CavDQnhK.js");return{runWasix:E}},[]),m=await WebAssembly.compile(c),M=n==="stdin"?[]:["-i","/public/test.txt"],D={};if(n!=="stdin"){const E=new TextEncoder;D["/public"]={"test.txt":E.encode(a)}}const W=n==="stdin"?new TextEncoder().encode(a):"",y=await(await g(m,{args:M,mount:D,stdin:W})).wait();y.ok?u({stdout:y.stdout||"",stderr:""}):k(`Exit code: ${y.code}
${y.stderr||""}`)}catch(g){k(`Wasmer Error: ${g.message}`),console.error(g)}o(!1)}async function P(){switch(t){case"wasmer":await Q();break;case"browserWasiShim":await te();break;case"wasmRunner":await q();break}}const j=Ge[t];return T`
    <div class="space-y-8">
